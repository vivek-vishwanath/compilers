...
public class IRInterpreter {
    ...
    private static class ProgramCounter {
    ...
    public static class Stats {
        ...
        public Map<OpCode, Integer> instructionCounts;
        ...
        public void update(IRInstruction instruction) {
            ...
            instructionCounts.compute(instruction.opCode, (k, current) -> current + 1);
        }

        public int getNonLabelInstructionCount() {
            return totalInstructionCount - instructionCounts.get(OpCode.LABEL);
        }
    }
    ...
    public void run() throws IRException {
        ...
        IRInstruction entryCall = new IRInstruction(OpCode.CALL, new IROperand[]{mainFunctionOperand}, -1);
        ...
        stats.instructionCounts.put(OpCode.CALL, -1);
    ...
    private Object binaryOperation(OpCode opCode, IRType type, Object y, Object z) {
    ...
    private void executeInstruction(IRInstruction instruction) throws IRException {
        ...
        switch (instruction.opCode) {
            ...
            case RETURN: {
                ...
                assert callInst.opCode == OpCode.CALLR;
                stack.pop();
    ...
    private Map<String, Object> buildVarMap(IRFunction function, ArrayList<Object> arguments) {
        ...
        for (IRVariableOperand variable: function.variables) {
            if (variable.type instanceof IRArrayType arrayType) {
                Object[] arr = new Object[arrayType.getSize()];
                varMap.put(variable.getName(), arr);
                Arrays.fill(arr, 0);
            } else {
                if (variable.type == IRIntType.get())
                    varMap.put(variable.getName(), 0);
                else
                    varMap.put(variable.getName(), 0f);
            }
        }
